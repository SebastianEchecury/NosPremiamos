...
namespace TECSO.FWK.DocumentsHelper.Excel
{
    public class ExcelHelper
    {
        ...
        internal static void FillSheet<T>(ExcelWorksheet sheets, List<T> list, string headerText = null, List<ExcelFieldModel> fields = null)
        {
            ...
            foreach (var item in list)
            {
                ...
                foreach (ExcelFieldModel field in fields)
                {
                    ...
                    {
                        //var name = prop.Name;
                        sheets.Cells[row, cell].Value = GetValueFormated(prop.GetValue(item), field.FormatText);
                        cell++;
                    }
                }
        ...
        internal static object GetValueFormated(object value, string formatText)
        {

            if (string.IsNullOrEmpty(formatText) && (value is DateTime || value is Nullable<DateTime>))
            {
    ...
    // where T: class, new()
    {
        ...
        public ExcelParameters<T> AddField(string name, string labelText = null, string formatText = null, Boolean allowSum = false)
        {
            Fields.Add(new ExcelFieldModel { Name = name, LabelText = labelText ?? name, FormatText = formatText, AllowSum = allowSum });
