...
export class AclService implements ConfigData {
	...
	constructor(
		...
		this.authService.getUserRoles().subscribe(permissions => {
		 
			this.setCurrrentUserRoles(permissions);
		});
		...
		this.authService.onCredentialUpdated$
			...
			.subscribe(permissions => this.setCurrrentUserRoles(permissions));
		...
		this.onAclUpdated$.subscribe(acl => {
			...
			this.permService.loadPermissions(acl.permissions, (permissionName, permissionStore) => !!permissionStore[permissionName]);

			this.PermissionService.loadPermissions(acl.permissions);
	...
	setCurrrentUserRoles(permissions: any): any {
		// update roles if the credential data has roles
		 
		this.aclModel.permissions = [];
		if (permissions != null) {
			...
			this.aclModel.permissions = permissions;
